      ...
      m_pDialogButton(nullptr), m_pExitButton(nullptr)
{
    ...
    m_pDialogButton->signal_clicked().connect(
        sigc::mem_fun(*this, &ManualWindow::on_dialog_button_clicked));

    // Connette il segnale "clicked" del pulsante "Exit".
    m_pExitButton->signal_clicked().connect(
        sigc::mem_fun(*this, &ManualWindow::on_exit_button_clicked));
}
...
void ManualWindow::on_dialog_button_clicked()
{
    ...
    pDialog->signal_response().connect(
        sigc::bind(sigc::mem_fun(*this, &ManualWindow::on_dialog_response), pDialog));

    // Mostra il dialogo in maniera asincrona.
    pDialog->show();
}

void ManualWindow::on_dialog_response(int response, Gtk::MessageDialog *pDialog)
{
    switch (response)
    {
...
void ManualWindow::on_exit_button_clicked()
{
    close();
}
